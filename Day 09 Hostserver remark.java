...
public class Hostserver extends Thread{
	...
	static CountDownLatch Police;
	static CountDownLatch Doc;
	// 낮 밤 스위치
	...
	//투표대상을 저장함, 중복표를 할수 없도록 체크하는 배열도 생성
	static ArrayList<Integer> day_vote = new ArrayList<Integer>();
	static ArrayList<Integer> duplicate_vote = new ArrayList<Integer>();
	static ArrayList<Integer> mafia_vote = new ArrayList<Integer>();
	static ArrayList<Integer> duplicate_mafia_vote = new ArrayList<Integer>();
	
	//이름으로 번호접근, 번호로 이름 접근하는 Hashtable
	...
	static public int vote_checker(ArrayList<Integer> arr) { // vote 결과를 알려준다 -> 최다득표를 return 동투표가 나오면 -1 return
		...
		try{
			...
			for(int i = 0; i < players.size(); i++) {
				...
				if(towho == "individual") {
						...
						instruction = "당신은 마피아입니다! 시민을 암살하고 도시를 무너뜨리세요. \n 특수기능 : *를 이용하여 마피아들 간 회의를 진행하세요. 다른 이들은 볼 수 없습니다! 밤에는 !암살을 사용하여 시민 암살에 대한 투표를 진행할 수 있습니다. \n"
								+ "ex) *지금 경찰이 누군지 알아낸 것 같은데? |  !암살 OOO";}
						else if (roles.get(i) == 2){
						instruction = "당신은 경찰입니다! 시민을 보호하고 도시를 지켜야합니다. 의심가는 인물을 조사해보세요.\n 특수기능 : 밤에 !조사를 사용하여 마피아에 대한 잠복조사를 진행할 수 있습니다. \n"
								+ "ex) !조사 OOO";}
						else if (roles.get(i) == 3) {
						instruction = "당신은 의사입니다! 암살당할 것 같은 시민에 대한 수슬을 집도하세요. 당신의 선택이 시민의 죽음을 막을 수 있습니다.\n 특수기능 : 밤에 +치료를 사용하여 마피아에 대한 잠복조사를 진행할 수 있습니다. \n"
								+ "ex) +치료 OOO";}
	...
	public void run() {
		...
		try{
			...
			while ((inputLine = br.readLine()) != null ) { // 클라이언트가 메세지 입력시마다 수행
				if(!idinput) {  //연결 후 한번만 노출(이름 설정)
					name = inputLine;  //이름 설정
					...
					latchEnter.countDown(); //인원 들어오면 -1
					continue;
				}
				...
				if(isday == true && inputLine.contains("!투표 ") && duplicate_vote.get(numb) == 0 && roles.get(numb) != 4) {
					...
					if(roles.get(value) == 4){// 사망자는 투표할 수 없다.
						out = players.get(numb).getOutputStream();
						writer = new PrintWriter(out, true);
					...
					latchday.countDown();
				} else if (isday == true && inputLine.contains("!투표 ") && duplicate_vote.get(numb) != 0) {//중복 투표 방지
					out = players.get(numb).getOutputStream();
					writer = new PrintWriter(out, true);
				...
				if(isday == false) {
					...
					if(inputLine.contains("!암살 ") && duplicate_mafia_vote.get(numb) == 0 && roles.get(numb) == 1) {
						...
						if(mafia_vote_counter == n_mafia) {
							System.out.println("m:" + n_mafia);
							latchNight.countDown();//  모든 마피아가 투표를 완료하면 latch -1
						}
						continue;
					} else if(isday == false && inputLine.contains("!암살 ") && duplicate_mafia_vote.get(numb) != 0){ //마피아 중복투표 체크
						out = players.get(numb).getOutputStream();
						writer = new PrintWriter(out, true);
						writer.println("이미 투표하셨습니다.");
						continue;
					...
					//경찰의 조사
						...
						if(inputLine.contains("!조사 ")) {
							...
							latchNight.countDown(); // 조사 완료하면 latch -1
							continue;
						}
					...
					//의사의 치료
						...
						if(inputLine.contains("!치료 ")) {
							...
							writer = new PrintWriter(out, true);
							writer.println("선택했습니다. 다른 플레이어들을 기다리고 있습니다.");
							latchNight.countDown();// 치료 완료하면 latch -1
							continue;
						}
					}
